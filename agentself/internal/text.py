...
def decode_utf8_text(data: bytes, *, strip_newline: bool = True) -> str:
    ...
    text = data.removeprefix(UTF8_BOM_BYTES).decode("utf-8")
    if strip_newline:
...
def read_text_file(path: Path, *, strip_newline: bool = True) -> str:
    return decode_utf8_text(Path(path).read_bytes(), strip_newline=strip_newline)
