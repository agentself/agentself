...
- Setup option `help` and connect `message` must be enough for an agent that has only JSON stdout, `backends`, and that text to choose the next action: obtain the value, continue with `--result-file`, ask a human, or `init --force` to another backend. Provider signup, OTP, URLs, and env aliases belong in `help` / `message`, not in the parser.
...
- Success and failure write one JSON object to stdout. stderr is empty for handled outcomes.

## Backend setup rules

- Keep provider details in the backend. Help text, URLs, validation, and API calls belong in the adapter; the core only handles generic options, actions, states, and results.
- Return the same setup result for every caller. Agents consume JSON.
...
4. Add a `CHANNELS` row in `agentself/host.py`. Set `live`, `verbs`, `custody`, `network`, `asset`, and `options`. Option fields are `name`, `type`, `required`, `sensitive`, `default`, `choices`, `source`, `help`, and the optional `action`.
