...
def test_diagnose_agentmail_token_canary_is_absent(tmp_path):
    ...
    sealed = run_cli(["secret", "create", "email.send.token", TOKEN_CANARY], env)
...
def test_diagnose_imap_address_ready_after_secrets(tmp_path):
    ...
    assert started.returncode == 0, started.stderr
    token = run_cli(["secret", "create", "email.send.token", TOKEN_CANARY], env)
    ...
    assert TOKEN_CANARY not in blob
    addr = run_cli(["secret", "create", "email.address", ADDRESS_CANARY], env)
